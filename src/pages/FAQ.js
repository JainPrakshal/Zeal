...
class SignUp extends Component {
    ...
    postFAQ = async () => {
        ...
        var errors = []
        await axios.post('/api/faq/', template)
            .then((data) => {
                if (errors.length == 0) {
                    this.setState({ new_faq_question: "" });
                    this.handleCreateModal()
                    this.loadFaqs(`/api/faq/?page=${this.state.page}&search=${this.state.search_query}`)
                }
            })
            .catch((error) => {
                const response_data = error.response.data
                ...
                <section className="d-flex align-items-center my-5 mt-lg-6 mb-lg-5">
                    <Container>
                        ...
                        <Row className={"m-3"}>
                            ...
                            <Col>
                            <Button variant="outline-success" onClick={() => {this.loadFaqs(`/api/faq/?page=${this.state.page}&search=${this.state.search_query}`)}}>
                                    <FontAwesomeIcon icon={faSearch} /> Search
                                </Button>
                            </Col>
                            <Col>
                            <Button variant="outline-danger" onClick={() => {this.loadFaqs(`/api/faq/`); this.setState({page: 1, search_query: ""})}}>
                                    <FontAwesomeIcon icon={faEraser} /> Clear
                                </Button>
                            </Col>
                            <Col>
                                <Button variant="outline-info" onClick={this.handleCreateModal}>
