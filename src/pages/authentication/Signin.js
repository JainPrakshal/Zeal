...
class SignUp extends Component {
  ...
  validateCredentials = async () => {
    ...
    await axios.post('/api/account/login', this.state.credentials)
      .then((response) => {
        if (errors.length == 0) {
          this.setState({ errors: [] });
          // window.open(Routes.DashboardOverview.path)
        }
      })
      .catch((error) => {
                  ...
                  <Form className="mt-4">
                    <Form.Group id="email" className="mb-4">
                      ...
                      <InputGroup>
                        ...
                        <Form.Control autoFocus required type="email" placeholder="example@company.com" onChange={(event) => { this.setCredentials("username", event.target.value) }} />
                      </InputGroup>
                    </Form.Group>
                    <Form.Group>
